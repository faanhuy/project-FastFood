...
public class AdminPartialRefundOrderCommandHandler(
    IOrderRepository orderRepository,
    IOrderStatusHistoryRepository statusHistoryRepository,
    IReturnRequestRepository returnRequestRepository,
    IPaymentGateway paymentGateway,
    IUnitOfWork unitOfWork) : IRequestHandler<AdminPartialRefundOrderCommand, OrderDto>
{
    public async Task<OrderDto> Handle(AdminPartialRefundOrderCommand request, CancellationToken cancellationToken)
    {
        ...
        // 5. Validate: selected items exist and calculate refund amount
        if (request.SelectedItemIds.Count == 0)
            throw new ConflictException("error.refund_select_items_required", null);

        var selectedItems = order.Items.Where(i => request.SelectedItemIds.Contains(i.Id)).ToList();

        if (selectedItems.Count != request.SelectedItemIds.Count)
            throw new ConflictException("error.refund_items_not_in_order", null);

        var refundAmount = selectedItems.Sum(i => i.UnitPrice * i.Quantity);

        if (refundAmount <= 0)
            throw new ConflictException("error.refund_amount_invalid", null);

        // 6. Process refund based on payment method
        ...
        {
            finalRefundNote = $"{finalRefundNote}\n[COD - Manual Partial Refund]".Trim();
        }

        // 7. Mark return request as refunded
        ...
        var statusHistory = OrderStatusHistory.Create(
            ...
            request.AdminUserId,
            $"Hoàn tiền một phần: {refundAmount} VNĐ - {finalRefundNote}");
