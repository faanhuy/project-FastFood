...
public class ImagesController(IWebHostEnvironment env) : ControllerBase
{
        ...
        IFormFile file, CancellationToken ct)
    {
        ...
        if (!AllowedExtensions.Contains(ext))
            return BadRequest(ApiResponse.Fail($"Chỉ chấp nhận: {string.Join(", ", AllowedExtensions)}"));

        if (file.Length > MaxBytes)
            return BadRequest(ApiResponse.Fail("Kích thước file không được vượt quá 5 MB."));
